...
### 1.1 At a Glance: Before vs. After

Every row below is traceable to a specific finding elsewhere in this document (section references given). "Before" is the current, verified state of the repository. "After" is the state once all nine phases in §11 are implemented.

| # | Dimension | Before | After |
|---|---|---|---|
| 1 | Active agent count | 127 agents (matches `plugin.json`'s own count: 129 files under `agents/` minus 2 non-agent files, `base-agent.md` and `ux/README.md`) | 126 — `autonomous-controller` and `sprint-loop` retired/folded in (§10.1, §11 Phase 3), one new agent `execution-ledger` added (§10.3, Phase 6): 127 − 2 + 1 = 126 |
| 2 | `/devteam:plan` | Calls only `research-agent`. Writes the PRD, tasks, and sprints itself, inline, with a thinner process than the dedicated agents define (§3, §2) | Calls `research-agent`, then `prd-generator`, then `task-graph-analyzer`, then `sprint-planner` in a real chain — each producing its full, richer output (§11 Phase 2) |
| 3 | `/devteam:implement` | Calls **zero** agents via `Task()`. Runs its own simplified, hand-written copy of the task loop (§4, §2) | Calls `orchestration:sprint-orchestrator`, which runs the real `task-loop` for every task (§11 Phase 3) |
| 4 | Orchestration depth | 4 nested layers designed (`autonomous-controller → sprint-orchestrator → sprint-loop → task-loop`); only `task-loop` is ever reachable, and only from `/devteam:bug`/`/devteam:issue` (§2, §4) | 3 layers (`sprint-orchestrator → task-loop → specialist agent`), all reachable from every implementation command (§10.1, Phase 3) |
| 5 | Autonomous controller | Exists as a file; called by nothing — no command, skill, or hook reaches it (§7) | Retired as a separate agent; "autonomous mode" becomes a flag `sprint-orchestrator` honors directly, enforced by the (already-working) Stop hook (§10.1, Phase 3) |
| 6 | UX / design work | Only runs if the user separately runs `/devteam:design`; never triggered by `/devteam:plan` or `/devteam:implement`; no `"design"` task type exists at all (§5) | Runs automatically — `task-graph-analyzer` emits a `design` task with a dependency edge, and `sprint-orchestrator` dispatches the design agent before any dependent UI task (§10.2, §10.1, Phase 4) |
| 7 | UI-writing agent (`frontend-developer.md`) | 1.9 KB generic checklist; never reads any design file; has no `Task` tool, so it can't ask for one (§5) | Rewritten to require reading the design-system output first, with real layout guidance added (§11 Phase 4) |
| 8 | Self-review before marking a task done | Does not exist anywhere. Only mechanical checks exist (did tests/lint pass) (§6) | Every agent must answer 3 questions (requested behavior / regressions / edge cases) and produce a completion report before it can finish (§11 Phase 5) |
| 9 | Requirement & scope checking (`requirements-validator`, `quality-gate-enforcer`, `scope-validator`) | Exist and are well-built, but only run for `/devteam:bug`/`/devteam:issue` — never for normal sprint/task work (§4, §6) | Run automatically for every task, from every command, because they sit inside `task-loop`, which is now the one real execution engine (§11 Phase 3) |
| 10 | Quality gates | Hardcoded to `npm test`/`npm run lint`/`npm run typecheck` only — breaks on non-JS projects (§4) | Detects the project's actual language/stack and runs the matching checks (pytest/mypy, go test/vet, etc.) (§11 Phase 3) |
| 11 | Hooks (`Stop`, `PreToolUse`, `SessionStart`, etc.) | **Correction to the common assumption:** these are real, working scripts, auto-installed with the plugin — not abandoned (§0, §7) | Unchanged — this part of the system was already solid; effort goes elsewhere |
| 12 | Cost tracking | Two separate, disagreeing records of the same cost — one in cents (`agent_runs`/`sessions`), one in dollars (`token_usage`/`cost-log.json`) (§7) | One single, agreed-upon cost record; the dollar-based path becomes a read-only display formatter over it (§11 Phase 0) |
| 13 | `config.yaml`'s autonomous-mode settings | Present in the file, but nothing reads it — changing `max_iterations` or `max_consecutive_failures` in the file has no effect (§7) | Actually parsed and applied at the start of every session (§11 Phase 0) |
| 14 | Execution logs / reports | Raw per-task data (agent, model, tokens, cost) already sits in the database, but nothing renders it into something readable — you have to run `/devteam:logs` and query it (§8) | The new `execution-ledger` agent automatically writes a plain-language report for every task and every sprint into a dedicated `devteam-reports/` folder (§10.3, Phase 6) |
| 15 | Which orchestrator ran which task | Not recorded anywhere — `agent_runs` has no field for "who dispatched this" (§8) | Recorded via a new database column, so reports can show the full chain, e.g. `sprint-orchestrator → task-loop → frontend:developer` (§10.3, Phase 1 + 6) |
| 16 | Planning file formats (PRD / task / sprint schemas) | The three planning agents each expect a slightly different format from one another, and none matches what `/devteam:implement` actually reads (§3) | One agreed format used by all three, and by the model-selection logic that depends on it (§11 Phase 1) |
| 17 | Agent registration | The same agent (`sprint-planner`) is registered twice, in two different config files, with two different, disagreeing settings (§3) | Registered once, in one place (§11 Phase 1) |
| 18 | Parallel work across multiple tracks (git worktrees) | The real, tested logic for this exists in an agent (`sprint-planner`) that is never called; what actually runs is a simpler, hand-written copy inside `devteam-implement.md` (§3, §4) | The tested version is what actually runs, dispatched by `sprint-orchestrator` (§11 Phase 2 + 3) |
| 19 | Two-phase (Ralph-style) continuous coding model (`features.json`/`progress.txt`/`init.sh`) | Fully built on the consumer side (`task-loop.md`, supporting scripts) but never triggered — `/devteam:plan` never produces the files it needs (§3) | Produced by the reconnected `prd-generator` and consumed by the reconnected `task-loop`, so it actually runs (§11 Phase 2 + 3) |
| 20 | Protection against this regressing again | None — this exact "agent built, never connected" problem already happened once before (Oct 2025, §9) and quietly crept back as the project grew from 28 to 127 agents | An automated check fails the build if any orchestration/planning agent is added without also being connected to something (§11 Phase 7) |
